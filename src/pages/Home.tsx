      ...
      <div className="relative z-10 space-y-4">
        <div className="flex justify-between items-start">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-bold text-sm text-gray-500">ورد الصلاة القادمة</h3>
              <p className="text-xl font-bold font-serif text-emerald-900">{activePrayerName}</p>
            </div>
          </div>
          <div className="text-left">
            <span className="text-[10px] bg-emerald-50 px-2 py-1 rounded-lg border border-emerald-100 font-bold text-emerald-700 uppercase">الآن</span>
          </div>
        </div>
        ...
        <div className="space-y-3 pt-2">
          {activeSlots.length === 0 ? (
            <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100 text-center">
              <p className="text-xs text-gray-400 italic">لا يوجد ورد مراجعة مخصص لصلاة {activePrayerName} حالياً.</p>
            </div>
          ) : (
            activeSlots.map((slot) => (
              <div key={slot.id} className="bg-gray-50 p-3 rounded-2xl border border-gray-100 flex items-center justify-between group hover:bg-emerald-50/50 transition-all border-r-4 border-r-amber-400">
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-right">
                    ...
                    <h4 className="text-[13px] font-bold text-gray-800">{slot.assignedContent}</h4>
                  </div>
                </div>
                <button
                  onClick={() => {
        ...
        <button
          ...
          className="w-full py-3 bg-emerald-50 hover:bg-emerald-100 rounded-2xl text-xs font-bold text-emerald-800 transition-colors border border-emerald-100"
        >
    ...
    >
      ...
      {userProfile.appTrack === "review_only" ? (
        ...
        <div className="space-y-6">
          ...
          {/* NEXT PRAYER FOCUS (Now below Control Panel) */}
          {nextPrayerFocusCard}
        ...
        <div className="space-y-6">
          ...
          {/* NEXT PRAYER FOCUS (Now below Repetition/Prayer times grid) */}
          {nextPrayerFocusCard}
